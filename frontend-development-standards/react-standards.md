...
### 组件大小限制
- **单个组件不超过200行**：超过200行的组件必须拆分为多个小组件
- **组件职责单一**：每个组件只负责一个具体的功能
- **Props数量限制**：组件props不超过8个，超过时使用对象props

### 组件拆分原则
- **按功能拆分**：将不同功能的UI拆分为不同的组件
- **按数据流拆分**：将数据获取、处理、展示分离
- **按交互拆分**：将不同的交互逻辑分离
- **按复用性拆分**：将可复用的部分提取为独立组件
- **按复杂度拆分**：将复杂的逻辑拆分为多个简单组件

### 自定义Hook拆分
- **Hook职责单一**：每个Hook只负责一个特定的功能
- **状态分离**：将不同状态的管理分离到不同的Hook
- **逻辑复用**：将可复用的逻辑提取为独立的Hook 