...
### 函数大小限制
- **单个函数不超过50行**：超过50行的函数必须拆分为多个小函数
- **函数职责单一**：每个函数只负责一个具体的功能
- **参数数量限制**：函数参数不超过5个，超过时使用对象参数

### 函数拆分原则
- **按功能拆分**：将不同功能的逻辑拆分为不同的函数
- **按层次拆分**：将高层逻辑和底层实现分离
- **按数据流拆分**：将数据处理的不同阶段分离
- **按错误处理拆分**：将正常逻辑和错误处理分离

### 工具函数组织
```
utils/user/
├── userValidation.ts    # 用户数据验证
├── userFormatting.ts    # 用户数据格式化
├── userTransformation.ts # 用户数据转换
└── index.ts            # 统一导出
``` 
