...
- **适用时机**：README、API 文档、技术规范、团队指南、代码注释等纯文档工作。
- **目标**：清晰、准确、可维护的文档，降低理解成本。

**阶段速查**

- `0 定义`：doc-spec.md 写文档目标、受众、核心内容、风格要求、验收标准。
- `1 分解`：按章节拆步骤（概览 → 快速开始 → 深入 → 参考 → FAQ）。
- `2 上下文`：贴现有文档、相关代码、团队模板，请 AI 给重写/微调/新增计划。
- `3 内容`：逐章节生成 Markdown + 代码示例，禁止编造不存在的 API。
- `4 自查`：完整性、准确性、可读性、可操作性、维护性。
- `5 Commit`：按章节切 commit，例如 `docs: update installation guide`。

**Prompt 片段**

```text
请把以下文档需求整理成 doc-spec.md，包含目标受众、核心内容、风格要求、验收标准。
---
按章节拆步骤，写清每步的内容要点、需要的素材、如何验证。
---
只生成 Step 1 的内容草稿：Markdown 格式 + 代码示例（带注释），标注需要补充的素材。
```

**Commit 策略示例**

- `docs: add quick start guide`
- `docs: update API reference with v2.0 changes`
- `docs: improve architecture diagram and explanation`
- `chore(docs): fix broken links and outdated examples`
