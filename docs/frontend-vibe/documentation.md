...
> 设计文档（design.md / architecture.md）是项目设计的单一事实源；spec.md、tasks.md 等属于一次任务的过程文档。每次修改都要明确：“设计文档是否需要更新？若不需要，为什么？”。
...
- 范围：<需要覆盖的模块/章节>
- 设计文档链接 / 章节：<路径或“待新建”>
- 生命周期定位：<长期设计事实源 / 一次性 spec / task note>
输出：
1. 文档目标（读者视角：读完能做什么）
2. 必须包含的内容（核心章节/信息点）
3. 可选内容（nice-to-have）
4. 风格要求（技术深度、语气、示例密度）
5. 验收标准（可读性、完整性、准确性）
6. 设计文档的更新策略（需要新增章节？替换旧内容？无需调整但需备忘？）
若信息不足，请列出需要澄清的问题。
```
...
- 需要的素材（代码示例、截图、链接）
- 完成后如何验证（自查清单）
确保顺序合理（概览 → 快速开始 → 深入 → 参考 → FAQ）；若某一步要更新设计文档，请写清楚对应章节名称与完成标记。
```
...
5. 本次任务是否会影响设计文档？若会，列出要同步的章节名称与更新方式（替换/新增/说明无需变更）。
...
5. 若章节会写入设计文档，请标注“设计文档同步点”及其位置（例如：design.md#数据流）。
...
5. 设计文档同步（本次内容是否已写入 design.md？若无变化，是否记录了原因？）
输出格式：
- 内容验收 checklist
- 可读性 checklist
- 维护提示
...
6. 设计文档同步情况（更新了哪些章节；若无需更新请写明理由，便于 reviewer 追溯）
```
...
### 场景 0：设计文档维护（design.md / architecture.md）

- **定位**：长期描述系统边界、数据流、依赖、关键决策，PR/新人培训引用的单一事实源。
- **重点**：记录“现状 + 决策原因 + 约束 + 未来扩展点”，避免复制实现细节；标注最后审阅时间。
- **验收**：读者能否凭设计文档理解模块职责并定位到对应代码文件？
- **风险**：实现已更新但设计文档未同步；或 spec/ticket 信息未合并进设计文档导致事实分裂。
